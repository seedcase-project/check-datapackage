...
human-friendly version of the problems (below we're using `rich` for
pretty-printing, which is bundled with `check-datapackage` as
`pretty_print()`):

```{python}
issues = cdp.check(properties=package_properties)
cdp.pretty_print(cdp.explain(issues))
```

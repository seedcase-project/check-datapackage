...
-   `strict`: Whether to include "SHOULD" checks in addition to "MUST"
    checks ones. Defaults to `False`.

::: callout-important
The Data Package standard uses language from [RFC
2119](https://www.ietf.org/rfc/rfc2119.txt) when defining it's checks.
...
though we use "required" for "MUST" in our documentation.
:::
...
be included in a compliant Data Package. By default, `check()` only
checks properties that "MUST" be included. To include properties that
"SHOULD" be included, set the `strict` argument to `True`. For example,
the `name` field of a Data Package "SHOULD" not contain special
characters. So running `check()` in strict mode on the following

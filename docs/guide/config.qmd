...
-   `extensions`: A list of extensions, which are additional checks
...
It is possible to add checks in addition to the ones defined in the
Data Package standard. We call these additional checks *extensions*. There are currently two types of extensions
supported: `CustomCheck` and `RequiredCheck`. You can add as many
`CustomCheck`s and `RequiredCheck`s to your `Config` as you want to fit your needs.
...
required, even when they aren't required by the Data Package standard with a `RequiredCheck`.
...
To apply this `RequiredCheck`, it should be added to the `Config` object passed to `check()` like shown below:

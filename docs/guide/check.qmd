...
The structure and content of the Data Package's `datapackage.json`
should follow the [Data Package standard](https://datapackage.org). This
standard defines the available properties at each level of the Data
Package's descriptor file, specifies which properties are required, and
describes the allowed values for each.

::: callout-note
Note that we use the word *descriptor* to refer to the JSON file
containing the Data Package's metadata, i.e., the `datapackage.json`
file. In contrast, we use the word *properties* to refer to the contents
of the Data Package's descriptor loaded into memory as a Python `dict`.
:::

`check-datapackage` provides the `check()` function to verify that a
Data Package's properties meets these requirements. `check()` takes a
Python `dict` as input, representing the properties in the descriptor,
and by default returns a list of `Issue` objects, each corresponding to
a failed check on a field in the properties. If you prefer the output as
an error, you can also run `check()` with `error=True`. When checking
the properties, you can customise the function's behaviour by specifying
issues to ignore and additional custom checks to perform.

By default, `check()` returns a list of `Issue` objects, each
corresponding to one failed check on one field in the properties. If
there are no failed checks, an empty list is returned.

In the example below, all required fields in the properties are present
...
Now let's remove the required `description` field from the properties

...
The structure and content of the Data Package's `datapackage.json` should follow the
[Data Package standard](https://datapackage.org). This standard defines
the available properties at each level of the Data Package's descriptor file, specifies
which properties are required, and describes the allowed values for
each.

::: callout-note
Note that we use the word *descriptor* to refer to the JSON file containing the Data Package's metadata, i.e., the
`datapackage.json` file. In contrast, we use the word *properties* to refer to the contents of the Data Package's descriptor loaded into memory as a Python `dict`.
:::
...
Python `dict` as input, representing the properties in the descriptor, and by default
returns a list of `Issue` objects, each corresponding to a failed check
on a field in the properties. If you prefer the output as an error, you
can also run `check()` with `error=True`. When checking the properties,

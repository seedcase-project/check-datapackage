...
are required, and what values are allowed.

This guide shows how to use the main function `check()` to run these
...
introducing the default checks and how to configure which checks run
with the `config` argument, and how to handle failed checks with the
`error` argument.
...
function raise an error if it finds any issues, see the [Stop program on
...
Now let's define `package_properties` without the required `description`
field and give a `name` with the wrong type (a number instead of a
string) and run `check()` again:

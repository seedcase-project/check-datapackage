...
[`read_properties()`](https://seedcase-soil.seedcase-project.org/docs/reference/read_properties.qmd)
after first parsing the source with
[`parse_source()`](https://seedcase-soil.seedcase-project.org/docs/reference/parse_source.qmd)
...
function give an error if it finds any issues, see the
...
The output now lists one `Issue` for the `name` field being of the wrong
type.
...
::: callout-tip
For more details on configuring checks, see the
[Configuring checks](config.qmd) guide or the
[`Config` reference documentation](/docs/reference/Config.qmd).
:::

...
can also run `check()` in error mode. When checking a descriptor, you
can customise the function’s behaviour by specifying issues to ignore
and additional custom checks to perform.

## Default mode

When you run `check()` in default mode, it returns a list of `Issue`
...
Now, if any checks fail, the function raises an error instead of
...
In the example above, we would expect four checks to fail: the package
...
The Data Package standard expresses both required and recommended rules.

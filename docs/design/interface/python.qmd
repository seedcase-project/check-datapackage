...
`read_properties()` reads metadata from that address into a `Properties`
value (a type alias for `dict[str, Any]`).

See
`parse_source()`
and
`read_properties()` from [`seedcase-soil`](https://github.com/seedcase-project/seedcase-soil)
for more details.

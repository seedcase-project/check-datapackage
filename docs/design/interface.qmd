...
descriptor against the Data Package standard, as well as any custom
configurations added to those checks. Below is the signature of the interface:
...
The `Config` class is described below. By default, `check()` does not trigger an
error when issues are found, but can be triggered when `error=True` is given. We
don't want `check-datapackage` to trigger errors by default because we want to
allow users the flexibility to decide when and how these checks should be
enforced and used. The output of this function is a list of `Issue` objects,
...
``` python
def explain(issues: list[Issue]) -> list[string]:
...
class Config:
    ...
    Attributes:
        ...
        rules (list[Rules]): Custom checks listed here will be done in addition
            to checks defined in the Data Package standard.
        version (list[string]) = ["v2", "v1"]: Version(s) of the Data Package
            standard to check against. Defaults to both v2 and v1.
        strict (bool) = False: Whether to check recommended as well as required
            fields. If True, recommended fields will also be checked.
    """
...
class Exclude:
    ...
    Attributes:
        target: [JSON path](https://en.wikipedia.org/wiki/JSONPath)
            to the field or fields in the input object where errors should be ignored,
            e.g., `$.resources[*].name`. Needs to match exactly the location of the error
            to be ignored in the Data Package standard.
        type: Either "required" or "pattern". The type of the check to ignore.
            If not provided, all types of checks will be ignored for the given
            target.

    Methods:
        matches(error: CheckError) -> bool: Returns True if the given error matches
...
Expresses a custom check, that inherits from the `Filter` class.
...
class Rules(Filter):
    ...
    Attributes:
        ...
        type: Either "required" or "pattern". The type of the check to ignore.
            If not provided, all types of checks will be ignored for the given
            target.
        message: A user-friendly explanation of what went wrong and any
            tips for fixing it.
        check: Function for doing the custom check. Takes a value and returns
...
class Issue:
    ...
    Attributes:
        ...
        issue (string): A description of what exactly the issue is.
        solution (string): A message suggesting a potential solution to the issue.
        type (string, optional) = ["required", "pattern"]: The type of the check that failed.
